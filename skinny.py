#-*- coding:utf-8 -*-

sbox_4 = [12,6,9,0,1,10,2,11,3,8,5,13,4,14,7,15]
sbox_4_inv = [3,4,6,8,12,10,1,14,9,2,5,7,0,11,13,15]

sbox_8 = []
sbox_8_inv = []

P = [0,1,2,3,7,4,5,6,10,11,8,9,13,14,15,12]
P_inv = [0,1,2,3,5,6,7,4,10,11,8,9,15,12,13,14]

TWEAKEY_P = [9,15,8,13,10,14,12,11,0,1,2,3,4,5,6,7]
TWEAKEY_P_inv = [8,9,10,11,12,13,14,15,2,0,4,7,6,3,5,1]

RC = [
        0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3E, 0x3D, 0x3B, 0x37, 0x2F,
        0x1E, 0x3C, 0x39, 0x33, 0x27, 0x0E, 0x1D, 0x3A, 0x35, 0x2B,
        0x16, 0x2C, 0x18, 0x30, 0x21, 0x02, 0x05, 0x0B, 0x17, 0x2E,
        0x1C, 0x38, 0x31, 0x23, 0x06, 0x0D, 0x1B, 0x36, 0x2D, 0x1A,
        0x34, 0x29, 0x12, 0x24, 0x08, 0x11, 0x22, 0x04, 0x09, 0x13,
        0x26, 0x0c, 0x19, 0x32, 0x25, 0x0a, 0x15, 0x2a, 0x14, 0x28,
        0x10, 0x20]

def ShiftRows(state, inverse=False):
    return [state[index] for index in P]

def MixColumns(state, inverse=False):
    for i in range(4):
        state[i:i+16:4] = MixColumn(state[i:i+16:4], inverse=inverse)
    return state

def MixColumn(col, inverse=False):
    col[1] ^= col[2]
    col[2] ^= col[0]
    col[3] ^= col[2]

    temp = col[3]
    col[3] = col[2]
    col[2] = col[1]
    col[1] = col[0]
    col[0] = temp
    return col

def AddKey(state, key):
    state = [a ^ b for a, b in zip(state[:8], key[:8])] + state[8:]
    key = [key[index] for index in TWEAKEY_P]
    return state, key

def AddConst(state, rno):
    state[0] ^= RC[rno] & 0xf
    state[4] ^= (RC[rno]>>4) & 0x3
    state[8] ^= 0x2
    return state

def skinny128128(plaintext, key, nrounds, skip_last_MC=False):
    key = list(key)
    state = list(plaintext)

    for i in xrange(nrounds):
        # SB
        state = [sbox_8[x] for x in state]
        # CONST
        state = AddConst(state, i)
        # AK
        state, key = AddKey(state, key)
        # SR
        state = ShiftRows(state)
        # MC
        if (not skip_last_MC) or i != nrounds - 1:
            state = MixColumns(state)
    return state

if __name__ == '__main__':
    ct = skinny128128(range(16), range(16), 10)
    print " ".join("%02x" % c for c in ct)

